...
* Added 3 services for cadvisor, node-exporter and nginx-exporter. cadvisor scrapes data from all the containers automatically. Exposaed it on the monitoring network without exposing any ports. Node-exporter pulls hardware and os-level metrics. Nginx-exporter converts nginx stats to make it prometheus scrapeable.
